    ...
    $(".next").click(function(){
        ...
        if (currentIndex === 1) {
              ...
              if (delivery === "ship" && address === "") {
                  errorBox.html("<div class='error-message text-danger mt-2'>Vui lòng nhập địa chỉ nhận hàng.</div>");
                  return;
              }
          }
...
async function submitOrder() {
    ...
    if (billFile) {
        ...
        const DRIVE_UPLOAD_URL = 'https://script.google.com/macros/s/AKfycbxhVP01FMgHWLUJE0wYAuCqlV7GSSwaXdwzVSAUccRIrAse2FkbbuDTqk_HmmbUEN4f/exec'; // 👈 thay bằng URL Apps Script của bạn
    ...
    const address = deliveryMethod === 'ship' ? $('#address').val() : '';
    ...
    formData.append("product4_qty", $('input[name="product4_qty"]').val() || 0);
    formData.append("product5_qty", $('input[name="product5_qty"]').val() || 0);
    ...
    formData.append("shipping_fee", getNumber(shippingFee));
    ...
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwoLQCAS-e2OGREDaRUS1J19OyP7IekgQjqZi3vY76DHCtJ3Va5gGT1TYABp7d-x8HWXQ/exec';
...
function payment(){
    ...
    document.getElementById('grand-total-payment').textContent = formatMoney (shippingFee + costproduct);
}
